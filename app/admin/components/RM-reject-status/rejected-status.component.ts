...
export class RejectedStatusComponent implements OnInit {
    ...
    rmRejectedStatus: RejectedStatus[] = [];
  ...
  getSubRemarksLists(){
    this.rejectedStatusService.getListOfStatusOfReject().subscribe(
      ...
      {
        this.rmRejectedStatus = response;
  ...
  onSaveClick()
  {
    if (this.newForm.valid)
    {
      ...
      this.rejectedStatusService.insertRejectStatus(this.newForm.value).subscribe((response) =>
      {
        ...
        this.rmRejectedStatus.push(p);

        //Reset the newForm
        this.newForm.reset();
        $("#newRMrejectedStatusCancelModal").trigger("click");
        this.calculateNoOfPages();
  ...
  onEditClick(event, RejectedStatusParam: RejectedStatus)
  {
    ...
    setTimeout(() =>
    {
      ...
      this.editIndex = this.rmRejectedStatus.indexOf(RejectedStatusParam);
  ...
  onUpdateClick()
  {
    if (this.editForm.valid)
    {
      ...
      this.rejectedStatusService.updateRejectedStatus(this.editForm.value).subscribe((response: RejectedStatus) =>
      {
        ...
        this.rmRejectedStatus[this.editIndex] = response;

        //Reset the editForm
        this.editForm.reset();
        $("#editRMrejectedStatusCancelModal").trigger("click");
      },
        (error) =>
        {
          console.log(error);
  ...
  onDeleteClick(event, RejectedStatus: RejectedStatus)
  {
    ...
    this.deleteIndex = this.rmRejectedStatus.indexOf(RejectedStatus);
  }
  ...
  onDeleteConfirmClick()
  {
    ...
    this.rejectedStatusService.deleteRejectedStatus(this.deleteRejectStatus.id).subscribe(
      (response) =>
      {
        ...
        this.rmRejectedStatus.splice(this.deleteIndex, 1);
  ...
  onFilterStatus(val) {
    if(!val){
      ...
      const status = this.rmRejectedStatus.filter(status => status.is_active === val);
      this.rmRejectedStatus = status;
      // this.showLoading = false;
      this.calculateNoOfPages();
    }
